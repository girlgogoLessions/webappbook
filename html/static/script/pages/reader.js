...
(function () {
  ...
  var Util = (() => {
    ...
    var StorageGetter = key => localStorage.getItem(prefix + key)
    var StorageSetter = (key, val) => localStorage.setItem(prefix + key, val)
    var getBSONP = (url, callback) => {
      return $.jsonp({
        url: url,
        cache: true,
  ...
  function ReaderModel () {
    ...
    //   }, 'json')
    // }
    var getCurChapterContentPromise = () => {
      return new Promise(function(resolve,reject){
  ...
  function EventHanlder () {
    ...
    // 上下翻页
    $('#prev-button').on('click', () => readerModel.prevChapter(readerUI))
      // todo 获得章节的翻页数据->把数据拿出来渲染
  
    $('#next-button').on('click', () => readerModel.nextChapter(readerUI))
  }
  main()
})()
